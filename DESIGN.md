...
| 4 | Carta | Los 79 platos en 6 pestañas + los 2 arreglos en tarjetas aparte. Pestañas = `tablist` real: flechas mueven la selección, Home/End saltan a los extremos, sólo la pestaña activa entra en el tabulador. Los seis paneles se renderizan siempre y los inactivos llevan `hidden`: cada `aria-controls` apunta a un id que existe y los 79 platos van completos en el HTML exportado, no sólo el apartado abierto. Línea de puntos entre plato y precio como en la carta impresa. Junto al panel, la foto de la carta en la vitrina como procedencia de los precios. En móvil las pestañas hacen scroll horizontal; la página no (`horizontal-scroll`) |
...
| Resultado | `public/photos/` | 51 ficheros: 25 nombres × 2 anchuras en WebP, más `fachada-og.jpg` — la `og:image` en JPEG, porque LinkedIn y algunos mensajeros no previsualizan WebP. Sólo estos van a la build |
